...
export default function ReceiptsModule() {
  ...
  const [formData, setFormData] = useState({ supplier: "", date: "", total: 0 })
  ...
  const handleOpenDialog = (receipt?: Receipt) => {
    if (receipt) {
      ...
      setFormData({ supplier: receipt.supplier, date: receipt.date, total: receipt.total })
    } else {
      setEditingId(null)
      setFormData({ supplier: "", date: new Date().toISOString().split("T")[0], total: 0 })
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await fetch(`/api/receipts/${editingId}`, { method: "PUT", body: JSON.stringify(formData) })
      } else {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await fetch(`/api/receipts/${id}`, { method: "PUT", body: JSON.stringify({ status: newStatus }) })
      fetchReceipts()
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <TableBody>
                {receipts.map((receipt) => (
                  <TableRow key={receipt.id}>
                    ...
                    <TableCell>
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[receipt.status as keyof typeof statusColors]}`}
                      >
                        {receipt.status}
                      </span>
                    </TableCell>
                    <TableCell className="flex gap-1">
                      {receipt.status === "Draft" && (
                        <Button
                          ...
                          onClick={() => handleStatusChange(receipt.id, "Validated")}
                        >
                          <CheckCircle className="w-4 h-4" />
                        </Button>
                      )}
                      <Button variant="ghost" size="sm" onClick={() => exportToPDF(receipt)}>
                        <FileText className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => exportToExcel(receipt)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? "Edit Receipt" : "Create New Receipt"}</DialogTitle>
          </DialogHeader>
